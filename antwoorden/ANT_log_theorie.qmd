...
Na het nemen van de logaritme hebben we van de machtswet $Y=aM^b$ het volgende gemaakt:
$$\log(Y)=\log(a)+b\cdot \log(M).$$
Dit is een lineaire vergelijking in 
$\log(M)$, met richtingscoëfficiënt b en snijpunt $log(a)$. Daarom levert een log-log-plot van een machtsverband een rechte lijn op.
:::
...
::: {#exr-machtsform}
Het totale metabolisme verloopt volgens de machtsformule $Y_{\text{totaal}} = aM^b$ , en de massa-specifieke intensiteit is gelijk aan $Y_{\text{totaal}}$ gedeeld door de massa $M$. Gebruik de rekenregels van de logaritme om een uitdrukking te vinden voor $\log(Y_{\text{specifiek}})$ in termen van $\log(M)$ en koppel dat aan de laatste plot die je gevonden hebt. Wat voor informatie over het lineaire verband kun je hieruit halen?
:::

::: { .callout-tip collapse="true"}
## Antwoord @exr-machtsform

We vinden
$$Y_{\text{specifiek}} = \frac{Y_{\text{totaal}}}{M}=\frac{aM^b}{M}=aM^{b-1}.$$
Hieruit concluderen we dat 
$$\log\left(Y_{\text{specifiek}}\right)=\log(a) + (b-1)\log(M).$$
Dit betekent dat de lijn voor de massa-specifieke intensiteit dezelfde intercep5 heeft als de lijn voor het totale metabolisme, maar een helling die precies 1 lager is.
:::
...
::: { .callout-tip collapse="true"}
## Antwoord @exr-exp_bepalen

```{r}
model_totaal <- lm(log10(massa_metabolisme) ~ log10(massa))
summary(model_totaal)

model_specifiek <- lm(log10(metabolisme) ~ log10(massa))
...
::: {#exr-exponenten}
Je vond zojuist twee exponenten: één voor het verband tussen massa en massa-specifiek metabolisme, en één voor massa en totaal metabolisme. Gebruik je antwoord bij exr-machtsform om deze aan elkaar te koppelen. Verklaar daarmee je bevindingen.
:::

::: { .callout-tip collapse="true"}
## Antwoord @exr-exponenten

Bij @exr-machtsform vonden we dat deze exponenten precies 1 van elkaar verschillen. Ook hier vinden we dat de gevonden hellingen 1 van elkaar verschillen.
:::
...
De motivatie om de logaritme verder te onderzoeken, kwam vanuit de pH-berekeningen die we eerder hebben uitgevoerd. We hebben gezien dat de logaritme ook in andere contexten voorkomt, maar nu keren we toch nog terug naar de context van concentraties. 
**Dit geeft al een link met iets wat we later in het jaar zullen behandelen, maar dit deel is niet strikt noodzakelijk om de logaritme goed te begrijpen. Geen zorgen dus, als je niet meer aan dit gedeelte toekomt.Het is uiteraard wel zinvol het te proberen!**
...
::: {.callout-tip collapse="true"}
## Voor de portfolio-ontwikkelingsgroep: fractale structuren
