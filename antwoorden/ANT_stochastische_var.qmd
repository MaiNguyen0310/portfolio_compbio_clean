...
In het praktische gedeelte bij deze opdracht wordt de stelling geponeerd dat we geen zekerheid ten aanzien van een hypothese kunnen krijgen in de empirische wetenschap. Dit is niet volledig correct. Denk aan het bekende voorbeeld van de zwarte zwaan. Stel dat we uitsluitend witte zwanen waarnemen. Daaruit kunnen we **niet** afleiden dat alle zwanen wit zijn; er kan immers een anders gekleurde zwaan bestaan die we simpelweg nog niet hebben waargenomen. De hypothese blijft dus onzeker, hoeveel witte zwanen we ook tellen. Maar één enkele niet-witte zwaan volstaat om haar te weerleggen, en de onzekerheid ten aanzien van de hypothese verdwijnt, we kunnen haar ontkennen. Dit is het geval voor logische hypotheses.
...
::: {#exr-chi2_waarden}
Stel dat maïs precies aan de verwachte 3:1-verdeling voldoet, dus dat $H_0$ waar is. Verwacht je dan dat alle studenten dezelfde $\chi^2$-waarden vinden, of nog steeds verschillende? Met andere woorden: ontstaat er ook onder $H_0$ een verdeling van $\chi^2$-waarden? Zo ja, dan is dat een speciale verdeling. Wat zou een goede naam voor die specifieke verdeling zijn?
:::

::: {.callout-tip collapse="true"}
## Antwoord @exr-chi2_waarden

Dat de nulhypothese waar is, betekent niet dat elke steekproef ook exact op de verhouding 3:1 uitkomt. Elke student telt een andere, toevallige steekproef zaden, dus door steekproeftoeval zal bijna niemand precies deze verhouding krijgen, ook al klopt de onderliggende 3:1-verhouding perfect. Er ontstaat dus een verdeling van $\chi^2$-waarden onder $H_0$. Deze specifieke verdeling heet de `chi-kwadraatverdeling`.
:::
...
Deze onzekerheid wordt nog beter voelbaar bij de p-waarde waarmee we het behouden van $H_0$ op baseren, die op haar beurt een functie van de stochastische variabele $\chi^2$ is. Hieronder zie je de verdeling van de p-waarden over de groepen.
...
::: {.callout-tip collapse="true"}
## Antwoord @exr-p_waarden

Omdat de p-waarde zelf stochastisch is, is ook onze conclusie (verwerpen of niet) afhankelijk van toeval in de steekproef. Eén p-waarde is dus geen hard, definitief bewijs, maar een uitkomst die van steekproef tot steekproef kan variëren.
:::
...
::: aside
Merk dus op dat we zelf bepalen hoe groot de type-I-fout is door de kritieke waarde $\alpha$ te kiezen! Wegens historische redenen kiezen we over het algemeen $\alpha=0.05$, maar als het in bepaalde situaties extra belangrijk is om extra zekerheid te hebben, kunnen we de waarde verlagen.
:::


## Maat voor onzekerheid: Entropie.

Er bestaan verschillende maten in de statistiek die de hoeveelheid onzekerheid kunnen duiden. Later in het jaar komen we bijvoorbeeld de standaardfout (SE) tegen. Nu gaan we kijken naar een andere zeer belangrijke wetenschappelijke maat voor onzekerheid, namelijk `entropie`. Entropie kan worden opgevat naast een maat voor onzekerheid, ook als een maat voor willekeur of wanorde en zoals we zullen zien ook biodiversiteit en positie-specifieke entropie in moleculaire biologie, en is alleen een functie van een kansverdeling.
...
Een hoge entropie wijst op grote onzekerheid. Dit treedt op wanneer de kansen relatief gelijk verdeeld zijn over de mogelijke uitkomsten zoals de zuivere dobbelsteen.
...
::: {#exr-entropie_uitleg}
Leg uit waarom de entropie afneemt bij een meer voorspelbare uitkomst.
:::

::: {.callout-tip collapse="true"}
## Antwoord @exr-entropie_uitleg

Bij een voorspelbare uitkomst is de kans geconcentreerd op één (of enkele) uitkomsten, dus is $p_i$ (bijna) 1 voor een bepaalde uitkomst $i$. Dit betekent dat $-log(p_i)$ voor deze uitkomst klein is. Hierdoor wordt ook de som in @eq-entropie-def klein.
:::
...
Leg uit waarom entropie een maat kan zijn voor ongelijkheid in punt 3. Wordt de waarde van de entropie groter of kleiner bij sociale ongelijkheid (Thiel-index)?
:::

::: {.callout-tip collapse="true"}
## Antwoord @exr-entropie_sociaal

Bij grote gelijkheid is inkomen/welvaart gelijk verdeeld over de bevolking, wat lastig te voorspellen maakt wie welk aandeel heeft. Dat komt overeen met hoge entropie. Bij ongelijkheid concentreert het inkomen zich bij een klein aantal mensen, dus daalt de entropie. 
:::

::: {#exr-entropie_biodiv}
Leg uit waarom zou entropie dan een maat zou zijn voor biodiversiteit? Heeft een hogere biodiversiteit een kleinere of grotere entropie?
:::

::: {.callout-tip collapse="true"}
## Antwoord @exr-entropie_biodiv

Hoge biodiversiteit betekent dat veel soorten ongeveer even vaak voorkomend. Het is dan moeilijk te voorspellen welke soort je tegenkomt, dus is er een hoge mate van verassing, oftewel entropie. Bij een lage biodiversiteit domineert één (of enkele) soort(en) het gebied, waardoor de uitkomst voorspelbaarder is en de entropie lager is.
:::
...
::: {.callout-tip collapse = "true"}
## Antwoord @exr-tabel_pos
Op posities 1, 2, 3, 5 en 7 hebben alle zes soorten dezelfde bouwsteen. Positie 4 verschilt het sterkst (bijna elke soort heeft er een andere letter), met positie 6 als tweede meest variabele positie.
:::
...
::: {.callout-tip collapse = "true"}
## Antwoord @exr-tabel_verassing

Bij posities 1, 2, 3, 5 en 7 kun je met zekerheid voorspellen welke bouwsteen er staat. Voor positie 4 staat er bij bijna elke soort een andere bouwsteen. Deze positie is dus onvoorspelbaar.
:::
...
::: {.callout-tip collapse = "true"}
## Antwoord @exr-tabel_entropie
De meest geconserveerde posities (entropie = 0) zijn 1, 2, 3, 5 en 7. De minst geconserveerde is positie 4 (hoogste entropie), gevolgd door positie 6.

We gebruiken de logaritme met grondtal 2, omdat de uitkomst dan in `bits` wordt uitgedrukt. Grondtal 10 of de natuurlijke logaritme hadden ook gewerkt, want de rangorde tussen de posities blijft identiek.
:::
...
::: {.callout-tip collapse = "true"}
## Antwoord @exr-pos_ident

Deze posities coderen waarschijnlijk voor onderdelen van een essentieel eiwit of gen. Een mutatie op zo'n plek zou de functie ernstig verstoren of zelfs lethaal zijn, waardoor natuurlijke selectie zulke mutaties wegselecteert. Op andere, minder kritieke posities kunnen mutaties minder schadelijk zijn en dus kunnen deze mutaties worden overgedragen.
:::
...
::: {.callout-tip collapse = "true"}
## Antwoord @exr-entropie_evo

Lage entropie op die posities betekent dat mutaties daar bijna altijd zijn weggeselecteerd. Dit is een sterk signaal dat die posities cruciaal zijn voor de functie van het gen tijdens de vroege ontwikkeling. Dat verschillende, ver uiteenlopende soorten na honderden miljoenen jaren evolutie nog steeds exact dezelfde bouwsteen hebben op die plek, kan bijna niet toevallig zijn; het wijst op een gedeelde voorouder waarvan dit stukje DNA vrijwel ongewijzigd is doorgegeven.
:::

### Entropie is optelbaar

We gaan nu gebruik maken van een belangrijke eigenschap van de logaritme. Er is iets vreemds aan de levende wereld. Aan de ene kant wordt ze steeds complexer: een historische groei in biodiversiteit, een rijkdom aan vorm en functie. Maar aan de andere kant herkennen we juist heldere structuur: soorten, genera en families laten zich netjes onderscheiden. Meer variatie en meer ordening tegelijk; dat lijkt een tegenspraak.
...
Dankzij de logaritme kunnen we dus de entropie *ontleden*. Meet je de vorm van een willekeurig organisme in de biosfeer, dan valt de onzekerheid over die vorm uiteen in twee vragen: *welke soort is dit?* en *hoe ziet dit individu eruit, gegeven die soort?*. Niet alle organismen in een soort zien er identiek uit. In formulevorm ziet deze partitie er als volgt uit:
...
::: {.callout-tip collapse="true"}
## Antwoord @exr-entropie_canal

Canalisatie verkleint de term $𝐻(fenotype∣soort)$.Het ontstaan van nieuwe soorten vergroot de term $𝐻(soort)$. De totale entropie $𝐻(fenotype)$ kan gelijk blijven terwijl beide termen veranderen, zolang de toename in de ene term precies wordt gecompenseerd door de afname in de andere.
:::

Al deze vragen illustreren dat een relatief eenvoudige functie zoals entropie ons veel theoretische inzichten kan verschaffen, zowel kwalitatief als kwantitatief, die anders niet of erg moeilijk te begrijpen zijn. In de ecologie maakt entropie het bovendien mogelijk om verschillende bronnen van biodiversiteit afzonderlijk te meten: de diversiteit binnen een gebied (alfadiversiteit), tussen gebieden (bètadiversiteit) en over het geheel (gammadiversiteit); precies dezelfde opsplitsing in een "binnen"- en een "tussen"-term die we hierboven tegenkwamen.

::: aside
Deze begrippen zul je later in het jaar behandelen bij de cursus `Ecologie 1` - het is voor dit portfolio niet nodig om deze al goed te begrijpen.
:::
...
::: aside
Als je klaar bent, kun je deze file plaatsen in bijbehorende assignment in Brightspace. De maispracticum opdracht is niet nodig, maar sla deze wel goed op met een back-up. 
:::

::: {.callout-tip collapse="true"}
## Voor de portfolio-ontwikkelingsgroep: entropie als brug tussen wiskunde en natuurkunde

We hebben entropie tot nu toe gebruikt als een maat voor onzekerheid, opgebouwd uit kansen. Maar dezelfde formule, $H = -\sum_i p_i \log(p_i)$, duikt op in verrassend veel verder van elkaar liggende hoeken van de wetenschap.  Dat is geen toeval.
...
Wie hiermee verder wil, krijgt een glimp van hoe één wiskundige vorm de taal van onzekerheid, informatie én de tweede hoofdwet van de thermodynamica tegelijk spreekt en is volgens velen het belangrijkste wetenschappelijke inzicht dat er is. Kippenvel!!
:::